  ...
  return (
    <div className="flex flex-col gap-6 items-center w-full max-w-2xl mx-auto px-2 sm:px-4 md:px-6">
      <Card className="w-full border-2 border-transparent hover:border-primary/60 bg-gradient-to-br from-white via-zinc-50 to-zinc-100 dark:from-zinc-900 dark:to-zinc-800 dark:via-zinc-950 group shadow-md">
        <CardHeader>
          <CardTitle className="text-lg md:text-2xl truncate max-w-full">{editMode && isAdmin ? (
            <Input
              value={editTournament?.name || ""}
              onChange={e => setEditTournament(et => et && { ...et, name: e.target.value })}
              className="mb-2"
            />
          ) : (
            tournament.name
        ...
        <CardContent className="flex flex-col gap-6">
          <div className="flex flex-col gap-1 text-sm text-muted-foreground">
            <div className="flex items-center gap-2">
              <span className="w-4 h-4" role="img" aria-label="calendar">📅</span>
              {editMode && isAdmin ? (
                <Input
                  type="date"
                  value={editTournament?.date.slice(0, 10) || ""}
                  onChange={e => setEditTournament(et => et && { ...et, date: e.target.value })}
            ...
            <div className="flex items-center gap-2">
              ...
              {editMode && isAdmin ? (
                <Input
                  ...
                  onChange={e => setEditTournament(et => et && { ...et, location: e.target.value })}
                  className="mb-2 max-w-xs"
                />
              ) : (
            ...
            {editMode && isAdmin ? (
              <Input
                ...
                onChange={e => setEditTournament(et => et && { ...et, description: e.target.value })}
          ...
          <div>
            <h3 className="font-semibold mt-4 mb-2 text-base md:text-lg">Players in this tournament:</h3>
              ...
              <table className="min-w-full border rounded text-sm md:text-base">
                ...
                <tbody>
                  {tournament.players.length === 0 && (
                    <tr><td colSpan={isAdmin ? 6 : 5} className="text-muted-foreground px-2 py-2">No players yet</td></tr>
                  )}
                  {tournament.players.map((p) => (
                    <PlayerRow
                      ...
                      onPlayerUpdated={async () => {
                        ...
                        const refetch = await fetch(`/api/tournaments/${tournament.id}`);
                        if (refetch.ok) {
                          const data = await refetch.json();
                          setTournament(data.tournament);
                          setEditTournament(data.tournament);
                  ...
                  {tournament.players.length > 0 && (
                    <tr className="font-bold bg-muted/60">
                      ...
                      <td className="px-2 py-1">{tournament.players.reduce((sum, p) => sum + (p.points ?? 0), 0)}</td>
                      <td className="px-2 py-1">{tournament.players.reduce((sum, p) => sum + (p.bounty ?? 0), 0)}</td>
                      <td className="px-2 py-1">{tournament.players.reduce((sum, p) => sum + (p.reentries ?? 0), 0)}</td>
                      {isAdmin && <td></td>}
                    </tr>
                  )}
                </tbody>
            ...
            {isAdmin && (
              <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-end mb-4 w-full mt-4">
                <div className="flex-1 min-w-0">
                  <PlayerAutocomplete
                    options={allPlayers.filter(p => !tournament.players.some(tp => tp.id === p.id))}
                    value={addPlayerIds}
          ...
          {isAdmin && (
            <div className="flex gap-2 mt-4 flex-wrap">
              {editMode ? (
                <>
                  <Button onClick={handleEditSave} disabled={saving} className="h-10 md:h-8">
                    {saving ? "Saving..." : "Save"}
                  </Button>
                  <Button variant="outline" onClick={() => { setEditMode(false); setEditTournament(tournament); }} className="h-10 md:h-8">
                    Cancel
                  </Button>
                </>
              ) : (
                <Button variant="outline" onClick={() => setEditMode(true)} className="h-10 md:h-8">
...
function PlayerRow({ player, tournamentId, isAdmin, onPlayerUpdated, onRequestRemove }: {
  player: Player;
  tournamentId: string;
  isAdmin: boolean;
  ...
  const [reentries, setReentries] = React.useState<number | "">(player.reentries ?? "");
  ...
  const handleSave = async () => {
    ...
    const res = await fetch(`/api/tournaments/${tournamentId}/players`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ playerId: player.id, rank: rank === "" ? null : Number(rank), points: points === "" ? null : Number(points), bounty: bounty === "" ? null : Number(bounty), reentries: reentries === "" ? null : Number(reentries) }),
    });
    setSaving(false);
    if (!res.ok) {
      const data = await res.json();
  ...
  let highlight = "";
  if (player.rank === 1) highlight = "bg-yellow-100 dark:bg-yellow-900/40 font-bold";
  else if (player.rank === 2) highlight = "bg-gray-100 dark:bg-gray-800 font-semibold";
  else if (player.rank === 3) highlight = "bg-orange-100 dark:bg-orange-900/40 font-semibold";

  if (!isAdmin) {
    return (
      <tr className={highlight + " border-b last:border-b-0"}>
        <td className="px-2 py-1 truncate max-w-[120px] md:max-w-[220px]">{player.name}</td>
  ...
  return (
    <tr className={highlight + " border-b last:border-b-0"}>
      <td className="px-2 py-1 truncate max-w-[120px] md:max-w-[220px] align-middle">{player.name}</td>
      ...
      <td className="px-2 py-1 align-middle flex flex-col gap-1 sm:flex-row sm:gap-2">
        {edit ? (
          <Button size="sm" onClick={handleSave} disabled={saving} className="mr-2 h-7">
            {saving ? "Saving..." : "Save"}
          </Button>
        ) : (
          <Button size="sm" variant="outline" onClick={() => setEdit(true)} className="mr-2 h-7">
            Edit
          </Button>
        )}
        {edit && (
          <Button size="sm" variant="ghost" onClick={() => { setEdit(false); setRank(player.rank ?? ""); setPoints(player.points ?? ""); setBounty(player.bounty ?? ""); setReentries(player.reentries ?? ""); }} className="h-7">
            Cancel
          </Button>
        )}
        <Button
